...
class Estimates_model extends Crud_model {
    ...
    function get_estimate_total_summary($estimate_id = 0) {
        ...
        $item_sql = "SELECT SUM($estimate_items_table.p_total) AS estimate_subtotal
    ...
    function save_descricao($descricao, $id) { 
        $estimates_table = $this->db->prefixTable('estimates'); 
        $sql = "UPDATE $estimates_table SET descricao = ? WHERE id = ?"; $params = [$descricao, $id];
         return $this->db->query($sql, $params); 
        }

    function save_introducao($introducao, $id) { 
        $estimates_table = $this->db->prefixTable('estimates'); 
        $sql = "UPDATE $estimates_table SET introducao = ? WHERE id = ?"; $params = [$introducao, $id];
            return $this->db->query($sql, $params); 
        }

    function save_observacao($observacao, $id) { 
        $estimates_table = $this->db->prefixTable('estimates'); 
        $sql = "UPDATE $estimates_table SET observacao = ? WHERE id = ?"; $params = [$observacao, $id];
            return $this->db->query($sql, $params); 
        }
                
    function save_pagamento($pagamento, $id) { 
        $estimates_table = $this->db->prefixTable('estimates'); 
        $sql = "UPDATE $estimates_table SET pagamento = ? WHERE id = ?"; $params = [$pagamento, $id];
            return $this->db->query($sql, $params); 
        }

    function save_config($data, $id) {
        $estimates_table = $this->db->prefixTable('estimates');
    
        // Constrói dinamicamente os campos para o SET
        $set_clause = [];
        $params = [];
        ...
        // Junta os campos para o SET em uma string
        $set_clause_string = implode(", ", $set_clause);
    
        // Constrói a consulta SQL
        $sql = "UPDATE $estimates_table SET $set_clause_string WHERE id = ?";
