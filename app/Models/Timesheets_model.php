...
class Timesheets_model extends Crud_model {
    ...
    function get_summary_details($options = array()) {
        ...
        $user_id = $this->_get_clean_value($options, "user_id");
        if ($user_id) {
           // $where .= " AND FIND_IN_SET($user_id, $timesheet_table.user_id)";
            $where .= " AND $timesheet_table.user_id=$user_id";
        }
    ...
    function get_timesheet_statistics($options = array()) {
        ...
        $soma_hora = " * (LENGTH($timesheet_table.user_id) - LENGTH(REPLACE($timesheet_table.user_id, ',', '')) + 1) AS";
        ...
        $timesheets_data = "SELECT DATE_FORMAT($select_tz_start_time,'%d') AS day, (SUM(TIME_TO_SEC(TIMEDIFF($select_tz_end_time,$select_tz_start_time))) + SUM(ROUND(($timesheet_table.hours * 60), 0) * 60)) $soma_hora total_sec
                FROM $timesheet_table 
...
$timesheet_users_data = "  SELECT $timesheet_table.user_id, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name,$users_table.image AS user_avatar, 
        (SUM(TIME_TO_SEC(TIMEDIFF($timesheet_table.end_time, $timesheet_table.start_time)))  + SUM(ROUND(($timesheet_table.hours * 60), 0) * 60)) AS total_sec
    FROM     $timesheet_table
    LEFT JOIN   $users_table  ON FIND_IN_SET($users_table.id, $timesheet_table.user_id)
    WHERE   $timesheet_table.deleted = 0 AND $timesheet_table.status = 'logged' $where
    ...
    function count_total_time($options = array()) {
        ...
        $attendance_sql = "SELECT  SUM(TIME_TO_SEC(TIMEDIFF($attendnace_table.out_time, $attendnace_table.in_time)) * 
        (LENGTH($attendnace_table.user_id) - LENGTH(REPLACE($attendnace_table.user_id, ',', '')) + 1)
    ) AS total_sec
FROM 
    $attendnace_table
WHERE 
    $attendnace_table.deleted = 0  AND $attendnace_table.status != 'incomplete' $attendance_where";
        $info->timecard_total = $this->db->query($attendance_sql)->getRow()->total_sec;


        $timesheet_sql = "SELECT (SUM(TIME_TO_SEC(TIMEDIFF($timesheet_table.end_time, $timesheet_table.start_time)) *
        (LENGTH($timesheet_table.user_id) - LENGTH(REPLACE($timesheet_table.user_id, ',', '')) + 1)) + SUM((ROUND(($timesheet_table.hours * 60), 0) * 60) * (LENGTH($timesheet_table.user_id) - LENGTH(REPLACE($timesheet_table.user_id, ',', '')) + 1))) AS total_sec
FROM 
    $timesheet_table
WHERE 
    $timesheet_table.deleted = 0 AND $timesheet_table.status = 'logged' $timesheet_where";
