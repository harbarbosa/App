...
class Estimates extends Security_Controller {
    ...
    function modal_form() {
        ...
        $view_data['taxes_dropdown'] = $this->Taxes_model->get_dropdown_list(array("title"));
    ...
    function save_descricao() {
        $this->access_only_allowed_members();

       

        
        $id = $this->request->getPost('estimate_id');
       
        $data = $this->request->getPost('descricao');
        ...
        $save_id = $this->Estimates_model->save_descricao($data, $id);
        if ($save_id) {
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $id);
        } else {
    ...
    function save_copy() {
       ...
       $list_data = $this->Estimate_items_model->get_details(array("estimate_id" => $id))->getResult();

       $delete = $this->Estimate_items_model->delete_all_items($estimate_id);

       foreach($list_data as $data){
        
        $bdi = 1 + ($data->bdi / 100);
        $p_unit = $bdi * $data->rate;
        ...
        $estimate_item_id = $this->Orcamento_items_model->ci_save($estimate_item_data, $id);
            ...
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $estimate_id);
        
    }

    function save_introducao() {
        $this->access_only_allowed_members();
        ...
        $data = $this->request->getPost('introducao');
        ...
        $save_id = $this->Estimates_model->save_introducao($data, $id);
        if ($save_id) {
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $id);
        } else {
    ...
    function save_observacao() {
        ...
        $data = $this->request->getPost('observacao');
        ...
        $save_id = $this->Estimates_model->save_observacao($data, $id);
        if ($save_id) {
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $id);
        } else {
    ...
    function save_pagamento() {
        ...
        $data = $this->request->getPost('pagamento');
        ...
        $save_id = $this->Estimates_model->save_pagamento($data, $id);
        if ($save_id) {
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $id);
        } else {
    ...
    function save_config() {
        ...
        $imposto_produto = unformat_currency($this->request->getPost('imposto_produto'));
        $imposto_servico = unformat_currency($this->request->getPost('imposto_servico'));
        $comissao_venda = unformat_currency($this->request->getPost('comissao_venda'));
        $impressao_total = $this->request->getPost('impressao_total');

        $data = array(
            "imposto_produto" => $imposto_produto,
            "imposto_servico" => $imposto_servico,
            "comissao_venda" => $comissao_venda,
            "impressao_total" => $impressao_total
            
            
        );
       
        
        $save_id = $this->Estimates_model->save_config($data, $id);
        if ($save_id) {
            $this->session->setFlashdata("success_message", app_lang('record_saved'));
            app_redirect("estimates/view/" . $id);
        } else {
    ...
    function view($estimate_id = 0) {
        ...
        if ($estimate_id) {
            ...
            $view_data["items"]  = $this->Estimate_items_model->get_details(array("estimate_id" => $estimate_id))->getResult();
    ...
    public function step_modal_form($estimate_id = 0)
{
    $estimate_id = $this->request->getPost('estimate_id');
    $view_data['estimate_id'] = $estimate_id;
    return $this->template->view('estimates/etapa_modal_form', $view_data);
}

public function save_step()
{
    $data = [
        'id_projeto' => $this->request->getPost('estimate_id'),
        'nome_etapa' => $this->request->getPost('step_name')
       
    ];

    $stepModel = new \App\Models\EtapaModel(); // Assumindo que você tenha um modelo para etapas
    $stepModel->ci_save($data);

    echo json_encode(array("success" => true, 'message' => "Etapa adicionada com sucesso!"));
}

//MODAL ADD EDIT ITEM ORÇAMENTO

function item_orcamento_modal_form() {
    ...
    $view_data['model_info'] = $this->Orcamento_items_model->get_one($this->request->getPost('id'));
    if (!$estimate_id) {
        $estimate_id = $view_data['model_info']->estimate_id;
    }
    $view_data['estimate_id'] = $estimate_id;
    return $this->template->view('estimates/item_orcamento_modal_form', $view_data);
}
    ...
    function save_item() {
        ...
        $estimate_bdi = $this->request->getPost('estimate_bdi');
        $item_id = 0;
        ...
        $estimate_item_data = array(
            ...
            "etapa_item" => $this->request->getPost('etapa_id'),
            "etapa_nome" => $this->request->getPost('estimate_item_etapa'),
            "description" => $this->request->getPost('estimate_item_description'),
            "quantity" => $quantity,
            "unit_type" => $this->request->getPost('estimate_unit_type'),
            "rate" => unformat_currency($this->request->getPost('estimate_item_rate')),
            "total" => $rate * $quantity,
            "p_total" => $rate * $quantity * $bdi,
            "bdi" => $estimate_bdi,
        );
    ...
    function save_item_orcamento() {
        ...
        $bdi = 1 + ($estimate_bdi / 100);
        $p_total = $quantity * unformat_currency($this->request->getPost('estimate_p_unit'));
        ...
        $estimate_item_data = array(
            ...
            "quantity" => $quantity,
            "unit_type" => $this->request->getPost('estimate_unit_type'),
            "p_unit" => unformat_currency($this->request->getPost('estimate_p_unit')),
            "p_total" => $p_total
        ...
        if ($estimate_item_id) {
            $options = array("id" => $estimate_item_id);
            $item_info = $this->Orcamento_items_model->get_details($options)->getRow();
            echo json_encode(array("success" => true, "estimate_id" => $item_info->estimate_id, "data" => $this->_make_item_row($item_info), "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), 'id' => $estimate_item_id, 'message' => app_lang('record_saved')));
        } else {
            echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
        }
    ...
    function orcamento_delete_item() {
        $this->access_only_allowed_members();
        ...
        if ($this->request->getPost('undo')) {
            if ($this->Orcamento_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Estimate_items_model->get_details($options)->getRow();
                echo json_encode(array("success" => true, "estimate_id" => $item_info->estimate_id, "data" => $this->_make_item_row($item_info), "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), "message" => app_lang('record_undone')));
            } else {
            ...
            if ($this->Orcamento_items_model->delete($id)) {
                $item_info = $this->Orcamento_items_model->get_one($id);
                echo json_encode(array("success" => true, "estimate_id" => $item_info->estimate_id, "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), 'message' => app_lang('record_deleted')));
            } else {
                echo json_encode(array("success" => false, 'message' => app_lang('record_cannot_be_deleted')));
            }
    ...
    function copy_items() {
        ...
        $view_data['estimate_id'] = $estimate_id;
        return $this->template->view('estimates/modal_copy_items', $view_data);
    }

    //LISTAR ETAPAS

    function etapa_list_data($estimate_id = 0) {
        validate_numeric_value($estimate_id);
        $this->validate_estimate_access($estimate_id);

        $list_data = $this->Estimate_items_model->get_etapa(array("estimate_id" => $estimate_id))->getResult();
        
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_item_row_etapa($data);
        }
        echo json_encode(array("data" => $result));
    }

    private function _make_item_row_etapa($data) {
        $move_icon = "";
        $desc_style = "";

       

        $item = "<div class='item-row strong mb5' data-id='$data->id_etapa'>$move_icon $data->nome_etapa</div>";
        ...
        return array(
            ...
            js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id_etapa, "data-action-url" => get_uri("estimates/delete_etapa/".$data->id_etapa), "data-action" => "delete"))
            
           
           // modal_anchor(get_uri("estimates/delete_etapa/".$data->id_etapa), "<i data-feather='x' class='icon-16'></i>", array("class" => "delete", 'title' => app_lang('delete'), "data-id" => $data->id_etapa, "data-action-url" => get_uri("estimates/delete_etapa", )))
    ...
    function delete_etapa($id) {
        ...
        $this->validate_submitted_data(array(
            ...
            if ($this->Estimate_items_model->delete_etapa($id)) {
    ...
    //LISTA ITENS ORÇAMENTO

    function item_orcamento_list($estimate_id = 0) {
        validate_numeric_value($estimate_id);
        $this->validate_estimate_access($estimate_id);

        $list_data = $this->Orcamento_items_model->get_details(array("estimate_id" => $estimate_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
            $etapas[] = $data->etapa_item;
            $result[] = $this->_make_item_row_orcamento($data);
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_item_row_orcamento($data) {
        $move_icon = "";
        $desc_style = "";
        ...
        return array(
            ...
            to_currency(($data->p_unit), $data->currency_symbol),
            to_currency(($data->p_total), $data->currency_symbol),
            modal_anchor(get_uri("estimates/item_orcamento_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_estimate'), "data-post-id" => $data->id, "data-post-estimate_id" => $data->estimate_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimates/orcamento_delete_item"), "data-action" => "delete"))
        );
    }
    ...
    private function _make_item_row($data) {
        ...
        return array(
            ...
            to_decimal_format($data->bdi),
            to_currency(((($data->bdi/100)+1) * $data->total), $data->currency_symbol),
    ...
    function get_estimate_etapa_suggestion() {
        $key = $this->request->getGet("q");
        $estimate_id = $this->request->getGet("estimate_id");
        $suggestion = array();

       

        $items = $this->Invoice_items_model->get_etapa_suggestion($key, $estimate_id);

        foreach ($items as $item) {
            $suggestion[] = array("id" => $item->id_etapa, "text" => $item->nome_etapa);
        }
    ...
    function get_estimate_etapa_info_suggestion() {
        $etapa = $this->Invoice_items_model->get_etapa_info_suggestion(array("etapa_id" => $this->request->getPost("etapa_id")));
        if ($etapa) {
                    echo json_encode(array("success" => true, "item_info" => $etapa));
        } else {
            echo json_encode(array("success" => false));
        }
    }
