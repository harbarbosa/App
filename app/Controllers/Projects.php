...
class Projects extends Security_Controller
{
    ...
    function diario_form() {
        ...
        $contexts = $TaskController->_get_accessible_contexts();
        ...
        foreach ($TaskController->get_context_id_pairs() as $obj) {
        ...
        $dropdowns = $TaskController->_get_task_related_dropdowns($selected_context, $selected_context_id, $selected_context_id ? true : false);
        ...
        if ($id) {
            ...
            if (!$TaskController->can_create_tasks()) {
                app_redirect("forbidden");
            }
        }
        ...
        $view_data["project_deadline"] = $TaskController->_get_project_deadline_for_task(get_array_value($view_data, "project_id"));
        $view_data["show_time_with_task"] = (get_setting("show_time_with_task_start_date_and_deadline")) ? true : false;
        $view_data['time_format_24_hours'] = get_setting("time_format") == "24_hours" ? true : false;

        $view_data["diario_model"] = $this->Timesheets_model->get_one($this->request->getPost('id'));

        return $this->template->view('projects/timesheets/diario_form', $view_data);
    }


    function diario_obra($project_id) {

        $TaskController = new Tasks();
        validate_numeric_value($project_id);

        if (!$TaskController->can_view_tasks("project", $project_id)) {
            app_redirect("forbidden");
        }
        ...
        $view_data['can_create_tasks'] = $TaskController->can_create_tasks("project");
        $view_data['can_edit_tasks'] = $TaskController->_can_edit_project_tasks($project_id);
        $view_data['can_delete_tasks'] = $TaskController->_can_delete_project_tasks($project_id);
        $view_data["show_milestone_info"] = $TaskController->can_view_milestones();

        $view_data['milestone_dropdown'] = $TaskController->_get_milestones_dropdown_list($project_id);
        $view_data['priorities_dropdown'] = $TaskController->_get_priorities_dropdown_list();
        $view_data['assigned_to_dropdown'] = $TaskController->_get_project_members_dropdown_list($project_id);
        $view_data["custom_field_headers"] = $TaskController->Custom_fields_model->get_custom_field_headers_for_table("tasks", $this->login_user->is_admin, $this->login_user->user_type);
        $view_data["custom_field_filters"] = $TaskController->Custom_fields_model->get_custom_field_filters("tasks", $this->login_user->is_admin, $this->login_user->user_type);

        $exclude_status_ids = $TaskController->get_removed_task_status_ids($project_id);
        ...
        return $this->template->view("projects/diario_obra/index", $view_data);
    }

    function diario_list_data($project_id = 0) {
        validate_numeric_value($project_id);
       

        $list_data = $this->Diario_obra_model->get_details(array("project_id" => $project_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
           
            $result[] = $this->_make_diario_row($data);
        }
        echo json_encode(array("data" => $result));
    }

    private function _make_diario_row($data) {
        $move_icon = "";
        $desc_style = "";
        ...
        $title = "<div class='item-row strong mb5' data-id='$data->id'>$move_icon $data->title</div>";

        $collaborators = $this->_get_collaborators($data->user_id);
        ...
        return array(
            ...
            modal_anchor(get_uri("estimates/item_modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_estimate'), "data-post-id" => $data->id, "data-post-estimate_id" => $data->project_id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_item"), "data-action" => "delete"))
        );
    }
    ...
    function save_item() {
        ...
        $id = $this->request->getPost('id');
        $rate = unformat_currency($this->request->getPost('project_item_rate'));
        $quantity = unformat_currency($this->request->getPost('project_item_quantity'));
        $project_item_title = $this->request->getPost('project_item_title');
        ...
        if ($add_new_item_to_library == 1) {
            $library_item_data = array(
                "title" => $project_item_title,
                "description" => $this->request->getPost('project_item_description'),
                "unit_type" => $this->request->getPost('project_unit_type'),
                "rate" => unformat_currency($this->request->getPost('project_item_rate')),
                "bdi" => $project_bdi
            );
        ...
        $project_item_data = array(
            "project_id" => $project_id,
            "title" => $this->request->getPost('project_item_title'),
            "data_retirada" => $this->request->getPost('data_entrega'),
            "description" => $this->request->getPost('project_item_description'),
            "quantity" => $quantity,
            "unit_type" => $this->request->getPost('project_unit_type'),
            "user_entrega" => unformat_currency($this->request->getPost('entregue_por')),
            "user_retirada" => unformat_currency($this->request->getPost('retirado_por')),
            "rate" => $rate,
           
        );

        if ($item_id) {
            $project_item_data["item_id"] = $item_id;
        }

        $project_item_id = $this->Project_items_model->ci_save($project_item_data, $id);
        if ($project_item_id) {
            $options = array("id" => $project_item_id);
            $item_info = $this->Project_items_model->get_details($options)->getRow();
            echo json_encode(array("success" => true, "project_id" => $item_info->project_id, "data" => $this->_make_item_row($item_info),  'id' => $project_item_id, 'message' => app_lang('record_saved')));
        } else {
     ...
     function item_modal_form() {
        ...
        if (!$project_id) {
            $project_id = $view_data['model_info']->project_id;
        }
        $view_data['project_id'] = $project_id;
        return $this->template->view('projects/materiais/item_modal_form', $view_data);
    }
    ...
    function delete_item() {
        ...
        $item_info = $this->Project_items_model->get_one($id);
      

        if ($this->request->getPost('undo')) {
            if ($this->Project_items_model->delete($id, true)) {
                $options = array("id" => $id);
                $item_info = $this->Project_items_model->get_details($options)->getRow();
                echo json_encode(array("success" => true, "project_id" => $item_info->project_id, "data" => $this->_make_item_row($item_info), "message" => app_lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, app_lang('error_occurred')));
            }
        } else {
            if ($this->Project_items_model->delete($id)) {
                $item_info = $this->Project_items_model->get_one($id);
                echo json_encode(array("success" => true, "project_id" => $item_info->project_id,  'message' => app_lang('record_deleted')));
            } else {
     ...
     function materiais($project_id)
     {
         ...
         if ($project_id) {
             ...
             return $this->template->view("projects/materiais/index", $view_data);
         }
     }

     function item_list_data($project_id = 0) {
        validate_numeric_value($project_id);
       

        $list_data = $this->Project_items_model->get_details(array("project_id" => $project_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function save()
    {
        ...
        $data = array(
            ...
            "ordem_servico" => $this->request->getPost('ordem_servico'),
    ...
    function overview($project_id)
    {
        ...
        $view_data["custos"] = $this->Tasks_model->get_task_statistics(array("project_id" => $project_id))->task_statuses;

       //CALCULO DE HORAS

      

            $result = $this->Timesheets_model->get_details(array("project_id" => $project_id));
            ...
            foreach ($list_data as $data) {
                ...
                $duration_in_seconds = $seconds * $qtArray;
    ...
    //CALCULO ITENS
        ...
        foreach($list_data_item as $item){

            $soma_itens = ($item->quantity * $item->rate) + $soma_itens;
        }
    //CALCULO GASTOS
        $options = array("project_id" => $project_id);
        $list_data_despesas = $this->Expenses_model->get_details($options)->getResult();

        $soma_despesa = 0;

        foreach($list_data_despesas as $despesa){

            $soma_despesa = $despesa->amount + $soma_despesa;
        }


        $project_data = $this->Projects_model->get_one($project_id);
    ...
    function timelog_modal_form()
    {
        ...
        $show_porject_members_dropdown = get_array_value($related_data, "show_porject_members_dropdown");
        $view_data["tasks_dropdown"] = get_array_value($related_data, "tasks_dropdown");
        $project_members = get_array_value($related_data, "project_members_dropdown");

        $project_members_dropdown = array();

        // Transformando o array original no formato correto
        foreach ($project_members as $member) {
            $ids = key($member); // Obtém a chave (ID)
            $name = $member[$ids]; // Obtém o valor (Nome)
        
            $project_members_dropdown[$ids] = $name; // Adiciona ao array final
        }
        ...
        if ($project_id) {
            ...
            if ($project_members) {
                foreach ($project_members as $member) {
                    ...
                    $project_members_dropdown[] = array($member->user_id => $member->member_name);
                }
            }
        } else {
    ...
    function save_timelog()
    {
        ...
        if (!$id) {
            // Inserção de novos dados, caso o id não exista
        ...
        if ($save_id) {
            ...
            // Retorna sucesso e dados da linha salva
            echo json_encode(array("success" => true, "data" => $this->_timesheet_row_data($save_id), 'id' => $save_id, 'message' => app_lang('record_saved')));
        } else {
    ...
    private function _make_timesheet_row($data, $custom_fields)
    {
        ...
        $start_time = $data->start_time;
        $end_time = $data->end_time;
        $project_title = anchor(get_uri("projects/view/" . $data->project_id));
    ...
    $duration_in_seconds = $seconds * $qtArray; // Multiplica o resultado em segundos pela variável
    ...
    $duration = convert_seconds_to_time_format($duration_in_seconds); // Converte para o formato desejado
        ...
        $options = modal_anchor(get_uri("projects/diario_form"), "<i data-feather='file' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_timelog'), "data-post-id" => $data->id))
                . modal_anchor(get_uri("projects/diario_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_timelog'), "data-post-id" => $data->id))
